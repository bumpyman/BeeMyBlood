...
5. Données du Testeur. L’Éditeur enregistre le nom, l’adresse courriel, l’organisation, le rôle déclaré, la date de signature du présent accord et un journal des connexions aux espaces. Ces données servent uniquement à gérer l’accès, à sécuriser la plateforme et à documenter le test auprès des instances de recherche et de financement. Elles sont hébergées en Suisse, chez Infomaniak à Genève pour le site et chez Supabase à Zurich pour les comptes, et conservées au plus trois ans après la fin de la phase alpha, puis supprimées ou anonymisées. Le Testeur peut demander l’accès, la rectification ou la suppression de ses données à contact@beemyblood.ch, conformément à la loi fédérale sur la protection des données (nLPD).
