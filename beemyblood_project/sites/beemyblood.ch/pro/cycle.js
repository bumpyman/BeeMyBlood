...
(function(){
  ...
  function codeAppel(x){ return String(x.id||'').replace(/-/g,'').slice(-4).toUpperCase(); }
  ...
  var ACTIONS = { reserve:[['confirme','Confirmer'],['arrive','Arrivé·e'],['noshow','Absent·e']], confirme:[['arrive','Arrivé·e'],['noshow','Absent·e']], arrive:[['questionnaire','Questionnaire vérifié'],['ajourne','Ajourner']], questionnaire:[['prelevement','Admettre au prélèvement'],['ajourne','Ajourner']], prelevement:[['termine','Don terminé']], termine:[], noshow:[['arrive','Finalement arrivé·e']], ajourne:[['arrive','Rouvrir le parcours']] };
  ...
  function rAgenda(){
    ...
    var rows = E.resa.map(function(x){ var d = donneur(x.donneur), ouvert = OUVERT===x.id, acts = ACTIONS[x.statut]||[]; return '<div class="cy-row'+(ouvert?' ouvert':'')+'" data-id="'+x.id+'"><div class="h">'+x.heure+'<small style="display:block;color:var(--slate-500);font-weight:400">lit '+x.lit+'</small></div><div class="n">'+h(nomDe(d))+'<small>'+(d?d.groupe:'')+' · '+(d?d.dons:0)+' dons'+(x.source==='externe'?' · en ligne':'')+'</small></div>'+badge(x.statut)+(ouvert?'<div class="cy-acts">'+(acts.length? acts.map(function(a){ return '<button class="cy-btn s'+(a[0]==='noshow'||a[0]==='ajourne'?'':' p')+'" data-st="'+a[0]+'" data-id="'+x.id+'">'+a[1]+'</button>'; }).join('') : '<span class="cy-muted">Parcours clos.</span>')+'<button class="cy-btn s" data-fermer="1">Fermer</button></div>':'')+'</div>'; }).join('');
    ...
    el.innerHTML = '<div class="grid-2-1"><div><div class="card">'+titre('Agenda et réservations','blue-400',TABS[0].ico)
      ...
      + '<p class="cy-muted" style="margin-bottom:.5rem">Les personnes réservent en ligne dans l’outil officiel du centre, vers lequel l’espace donneur redirige. L’agenda interne sert aux appels, aux passages spontanés et au suivi du parcours. Cliquez sur une ligne pour afficher les actions possibles (confirmer, arrivée, questionnaire, prélèvement, don terminé, absence).</p>'
      + '<div class="cy-list">'+(rows||'<p class="cy-muted">Aucune réservation aujourd’hui.</p>')+'</div></div></div>'
      + '<div><div class="cy-sticky"><div class="sidebar-card"><h4 style="font-weight:700;margin-bottom:.5rem">Liste d’attente</h4><p class="cy-muted" style="margin-bottom:.5rem">Un clic place la personne sur le premier créneau libre.</p><div class="cy-list">'+(attente||'<p class="cy-muted">Vide.</p>')+'</div></div>'
      + '<div class="sidebar-card" style="margin-top:1rem"><h4 style="font-weight:700;margin-bottom:.5rem">Capacité</h4><p class="cy-muted">'+p.lits+' lits, créneaux de '+p.creneau_min+' minutes de '+h(p.ouverture)+' à '+h(p.fermeture)+'. Absences estimées aujourd’hui : '+noShowPred+'. Les rappels à J-7 et J-1 partent automatiquement, avec confirmation en un clic.</p></div></div></div></div>';
    el.querySelectorAll('.cy-row[data-id]').forEach(function(row){ row.onclick = function(ev){ if(ev.target.closest('button')) return; OUVERT = (OUVERT===row.dataset.id) ? null : row.dataset.id; rAgenda(); }; });
    el.querySelectorAll('button[data-fermer]').forEach(function(b){ b.onclick = function(){ OUVERT=null; rAgenda(); }; });
    el.querySelectorAll('button[data-st][data-id]').forEach(function(b){ b.onclick = function(){ var x = resaDe(b.dataset.id), suivant = b.dataset.st; if(!x) return; OUVERT = null;
      agir('cd_reservation_statut', { p_id:x.id, p_statut:suivant, p_extra:{} }, function(){ x.statut=suivant; journalLocal(E, qui(), LIB[suivant]+' — '+nomDe(donneur(x.donneur)), 'Réservation '+x.id); if(suivant==='termine'){ var d=donneur(x.donneur); d.dons++; d.dernier=new Date(); } }, 'agenda', function(){ toast('success', LIB[suivant]+' — enregistré.'); }); }; });
  ...
  function rDonneurs(){
    ...
    el.innerHTML = '<div class="grid-2-1"><div><div class="card">'+titre('Dossier donneur·se','purple-400',TABS[1].ico)
      ...
      + '<div><div class="cy-sticky" id="cy-fiche">'+(fiche||'<div class="sidebar-card"><h4 style="font-weight:700;margin-bottom:.4rem">Fiche</h4><p class="cy-muted">Choisissez une personne dans la liste pour ouvrir sa fiche, la placer sur la liste d’attente ou lui envoyer un rappel.</p></div>')+'</div></div></div>';
    ...
    el.querySelectorAll('tbody tr').forEach(function(tr){ tr.onclick = function(){ SEL = tr.dataset.id; NOUVEAU=false; rDonneurs(); var f=document.getElementById('cy-fiche'); if(f && window.innerWidth<900) f.scrollIntoView({behavior:'smooth',block:'start'}); }; });
  ...
  function rAccueil(){
    ...
    if(a.resa){
      ...
      if(d){
        ...
        contenu = '<div class="cy-box" style="display:flex;justify-content:space-between;align-items:center;gap:1rem"><div><h6 style="margin:0">Code d’appel à remettre à la personne</h6><p class="cy-muted">C’est ce code, et lui seul, qui apparaît sur l’écran Flux donneurs de la salle d’attente.</p></div><b style="font-size:1.8rem;letter-spacing:.15em;color:var(--amber-400);font-variant-numeric:tabular-nums">'+codeAppel(x)+'</b></div>'
          + '<div class="cy-box"><h6>Questionnaire pré-rempli '+badge(x.statut)+'</h6><p class="cy-muted">Rempli en ligne la veille par '+h(nomDe(d))+'. Les réponses qui changent depuis le dernier don sont surlignées à la validation.</p>'
    ...
    el.innerHTML = '<div class="grid-2-1"><div><div class="card">'+titre('Accueil et consentement','green-400',TABS[2].ico)
      ...
      + '</div></div><div><div class="sidebar-card" style="margin-bottom:1rem"><h4 style="font-weight:700;margin-bottom:.4rem">Flux donneurs</h4><p class="cy-muted" style="margin-bottom:.5rem">L’écran de la salle d’attente : chaque personne y suit son tour, désignée uniquement par le code d’appel remis à l’accueil. Aucun nom, aucune initiale, aucune heure de rendez-vous n’y apparaît, et l’écran ne reçoit du serveur que les codes et les statuts. À ouvrir sur un grand écran ou un second poste ; il se met à jour toutes les 20 secondes.</p><a class="cy-btn p s" href="salle/?region='+cle()+'" target="_blank" rel="noopener">↗ Ouvrir le Flux donneurs</a></div><div class="sidebar-card"><h4 style="font-weight:700;margin-bottom:.4rem">Le parcours à l’accueil</h4><ol class="cy-muted" style="padding-left:1.1rem;line-height:1.7"><li>Identification par QR, e-ID ou nom</li><li>Questionnaire pré-rempli, vérifié avec la personne</li><li>Consentement électronique, confirmé par code</li><li>Mesures (hémoglobine, poids) et décision d’aptitude</li><li>Admission au prélèvement, fin de parcours consignée</li></ol></div></div></div>';
