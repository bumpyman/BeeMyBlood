...
(function(){
  ...
  var EPISODES = {
    pandemie: { nom:'Pandémie', desc:'Dons en baisse, demande programmée réduite puis rebond, ajournements en hausse.', champs:[['debut','Début',20,'jour'],['duree','Durée (jours)',56],['dons','Dons (%)',-40],['demande','Commandes (%)',-25],['ajourn','Ajournements (+ points)',5]] },
    ete:      { nom:'Vacances d’été', desc:'Moins de donneur·se·s présent·e·s, demande stable.', champs:[['debut','Début',30,'jour'],['duree','Durée (jours)',60],['dons','Dons (%)',-25],['demande','Commandes (%)',0],['ajourn','Ajournements (+ points)',0]] },
    canicule: { nom:'Canicule', desc:'Présentation en baisse, ajournements en hausse, demande légèrement accrue.', champs:[['debut','Début',40,'jour'],['duree','Durée (jours)',10],['dons','Dons (%)',-15],['demande','Commandes (%)',5],['ajourn','Ajournements (+ points)',8]] },
    campagne: { nom:'Campagne ou appel', desc:'Afflux de dons pendant quelques jours. La simulation montre l’effet sur la péremption cinq à six semaines plus tard.', champs:[['debut','Début',30,'jour'],['duree','Durée (jours)',7],['dons','Dons (%)',40],['demande','Commandes (%)',0],['ajourn','Ajournements (+ points)',2]] },
    accident: { nom:'Accident majeur', desc:'Demande vitale immédiate de concentrés érythrocytaires, substitution autorisée.', champs:[['jour','Jour',45,'jour'],['heure','Heure (0 à 23)',14],['cgr','Poches demandées',60]] },
    rupture:  { nom:'Rupture d’un groupe', desc:'Le stock d’un groupe tombe à zéro et ses donneur·se·s répondent peu pendant deux semaines.', champs:[['jour','Jour',30,'jour'],['groupe','Groupe sanguin',4,'groupe'],['reponse','Dons du groupe (%)',-70]] },
    labo:     { nom:'Panne du laboratoire', desc:'Le délai de qualification des poches est multiplié.', champs:[['debut','Début',50,'jour'],['duree','Durée (jours)',5],['facteur','Délai multiplié par',3]] },
    fetes:    { nom:'Fêtes de fin d’année', desc:'Dons en forte baisse pendant les fêtes, demande maintenue.', champs:[['debut','Début',80,'jour'],['duree','Durée (jours)',14],['dons','Dons (%)',-40],['demande','Commandes (%)',0],['ajourn','Ajournements (+ points)',3]] },
    relais:   { nom:'Relais communautaire', desc:'Supporters d’un club, entreprise ou bouche-à-oreille de patient·e·s : afflux court et ciblé.', champs:[['debut','Début',35,'jour'],['duree','Durée (jours)',3],['dons','Dons (%)',30],['demande','Commandes (%)',0],['ajourn','Ajournements (+ points)',4]] },
    mobile:   { nom:'Collecte mobile supplémentaire', desc:'Une collecte de plus, chaque semaine, le jour choisi.', champs:[['jourSemaine','Jour de la semaine',1,'jourSemaine'],['dons','Dons supplémentaires',25]] }
  };
  ...
  function vueResultats(){
    ...
    var table = '<div class="jm-wrap"><table class="jm-table"><thead><tr><th>Groupe</th><th>Couverture moyenne</th><th>Creux habituel</th><th>Jours sous '+sc.seuilTension+' j</th><th>Poches manquantes</th><th>Retour au-dessus du seuil</th></tr></thead><tbody>'
      + GROUPES.map(function(g){ var c=k.couv[g]; return '<tr><td><span class="jm-dot" style="background:'+COUL[g]+'"></span>'+g+'</td><td style="color:'+couleurCouv(c.moy,sc)+'">'+pm(c,' j')+'</td><td>'+f1(k.cMin[g].p10)+' j</td><td>'+pm(k.sous[g])+'</td><td>'+pm(k.missing[g])+'</td><td>'+(k.reappro[g].n? 'en général '+f1(k.reappro[g].med)+' j, au pire '+f1(k.reappro[g].p90)+' j' : 'jamais passé sous '+sc.seuilCritique+' j')+'</td></tr>'; }).join('')+'</tbody></table></div>';
    ...
    return '<div class="card"><div class="jm-cart">Simulation <b>'+r.id+'</b> · '+h(r.nom)+' · '+r.region+' · '+sc.horizon+' jours mesurés, rejoués '+r.n+' fois · tirage n° '+sc.graine+' · '+r.quand.toLocaleString('fr-CH')+'<br>Chaque chiffre est la moyenne des '+r.n+' simulations, suivie de sa marge d’incertitude (±). La valeur réelle se situe dans cette marge dix-neuf fois sur vingt. Cohérence interne vérifiée.</div>'
      ...
      + '<div class="jm-h">Détail par groupe sanguin</div>'+table
