...
(function(){
  ...
  function vueResultats(){
    ...
    return '<div class="card"><div class="jm-cart">Simulation <b>'+r.id+'</b> · '+h(r.nom)+' · '+r.region+' · '+sc.horizon+' jours mesurés, rejoués '+r.n+' fois · tirage n° '+sc.graine+' · '+r.quand.toLocaleString('fr-CH')+'<br>Chaque chiffre est la moyenne des '+r.n+' simulations, suivie de sa marge d’incertitude (±). La valeur réelle se situe dans cette marge dix-neuf fois sur vingt. Cohérence interne vérifiée.</div>'
      + kpis
      + '<div class="jm-h">Stock par groupe, jour après jour</div><p class="jm-muted" style="margin-bottom:.4rem">La ligne est la valeur la plus probable ; la zone colorée est la fourchette dans laquelle le stock se trouve huit fois sur dix. Les zones rouge et orange marquent les seuils de couverture.</p><div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.4rem"><select class="jm-in" id="jm-g" style="width:auto">'+GROUPES.map(function(g){ return '<option'+(UI.groupe===g?' selected':'')+'>'+g+'</option>'; }).join('')+'</select><span class="jm-muted">Seuils : '+sc.seuilTension+' j tendu, '+sc.seuilCritique+' j critique.</span></div><canvas class="jm-canvas" id="jm-c1"></canvas>'
      + '<div class="jm-h">Couverture par semaine et par groupe</div><p class="jm-muted" style="margin-bottom:.4rem">Vert : plus de '+sc.seuilTension+' jours de couverture. Orange : tension. Rouge : moins de '+sc.seuilCritique+' jours, risque de pénurie.</p>'+heat
