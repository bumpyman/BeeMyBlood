...
(function(){
  ...
  var EPISODES = {
    pandemie: { nom:'Pandémie', desc:'Moins de dons, commandes programmées réduites puis rattrapage, plus d’ajournements.', champs:[['debut','Début',20,'jour'],['duree','Durée (jours)',56],['dons','Variation des dons (%)',-40],['demande','Variation des commandes (%)',-25],['ajourn','Ajournements en plus (points de %)',5]] },
    ete:      { nom:'Vacances d’été', desc:'Moins de personnes viennent donner, les commandes restent les mêmes.', champs:[['debut','Début',30,'jour'],['duree','Durée (jours)',60],['dons','Variation des dons (%)',-25],['demande','Variation des commandes (%)',0],['ajourn','Ajournements en plus (points de %)',0]] },
    canicule: { nom:'Canicule', desc:'Moins de personnes viennent donner, plus d’ajournements, commandes légèrement en hausse.', champs:[['debut','Début',40,'jour'],['duree','Durée (jours)',10],['dons','Variation des dons (%)',-15],['demande','Variation des commandes (%)',5],['ajourn','Ajournements en plus (points de %)',8]] },
    campagne: { nom:'Campagne ou appel', desc:'Afflux de dons pendant quelques jours. La simulation montre l’effet sur la péremption cinq à six semaines plus tard.', champs:[['debut','Début',30,'jour'],['duree','Durée (jours)',7],['dons','Variation des dons (%)',40],['demande','Variation des commandes (%)',0],['ajourn','Ajournements en plus (points de %)',2]] },
    accident: { nom:'Accident majeur', desc:'Besoin immédiat de poches de globules rouges pour des patients en danger vital, substitution autorisée.', champs:[['jour','Jour',45,'jour'],['heure','Heure (0 à 23)',14],['cgr','Poches demandées en urgence',60]] },
    rupture:  { nom:'Rupture d’un groupe', desc:'Le stock d’un groupe tombe à zéro et ses donneur·se·s répondent peu pendant deux semaines.', champs:[['jour','Jour',30,'jour'],['groupe','Groupe sanguin',4,'groupe'],['reponse','Variation des dons de ce groupe (%)',-70]] },
    labo:     { nom:'Panne du laboratoire', desc:'Les analyses prennent plus de temps, les poches arrivent plus tard en stock.', champs:[['debut','Début',50,'jour'],['duree','Durée (jours)',5],['facteur','Délai des analyses multiplié par',3]] },
    fetes:    { nom:'Fêtes de fin d’année', desc:'Beaucoup moins de dons pendant les fêtes, commandes maintenues.', champs:[['debut','Début',80,'jour'],['duree','Durée (jours)',14],['dons','Variation des dons (%)',-40],['demande','Variation des commandes (%)',0],['ajourn','Ajournements en plus (points de %)',3]] },
    relais:   { nom:'Relais communautaire', desc:'Supporters d’un club, entreprise ou bouche-à-oreille de patient·e·s : un afflux de dons court et ciblé.', champs:[['debut','Début',35,'jour'],['duree','Durée (jours)',3],['dons','Variation des dons (%)',30],['demande','Variation des commandes (%)',0],['ajourn','Ajournements en plus (points de %)',4]] },
    mobile:   { nom:'Collecte mobile supplémentaire', desc:'Une collecte de plus, chaque semaine, le jour choisi.', champs:[['jourSemaine','Jour de la semaine',1,'jourSemaine'],['dons','Dons supplémentaires ce jour-là',25]] }
  };
  ...
  function scenarioDefaut(){
    ...
    return { nom:'Situation de référence', horizon:120, chauffe:30, graine:42, reps:10, donsJour:base, ajournement:12, demandeJour:Math.round(base*0.8), dureeVie:42, qualifH:30, cibleJours:7, seuilCritique:3, seuilTension:5, substitution:'urgences', profilDons:[1.05,1.1,1.05,1,.95,.55,.15], profilDemande:[1.1,1.1,1.1,1.05,1,.6,.5], episodes:[] };
  }
  ...
  function dateAxe(d){ var x=new Date(); x.setDate(x.getDate()+d); return x.toLocaleDateString('fr-CH',{day:'numeric',month:'short'}); }

  var css = '.jm-intro{font-size:.82rem;color:var(--slate-400);line-height:1.55;margin-bottom:.7rem}.jm-intro b{color:#f0ece2}.jm-guide{font-size:.8rem;color:var(--slate-400);line-height:1.55;padding:.6rem .8rem;border-left:3px solid var(--amber-500);background:rgba(232,213,163,.05);border-radius:.4rem;margin-bottom:.9rem}.jm-guide b{display:block;color:#f0ece2;margin-bottom:.15rem}.jm-nav{display:flex;gap:.4rem;overflow-x:auto;padding-bottom:.4rem;margin-bottom:.8rem;scrollbar-width:none}.jm-nav button{flex:none;padding:.45rem .8rem;border-radius:99px;border:1px solid var(--slate-700);background:var(--slate-800);color:#f0ece2;font:inherit;font-size:.78rem;font-weight:600;cursor:pointer}.jm-nav button.on{background:var(--amber-500);color:#0a0a12;border-color:var(--amber-500)}'
    ...
    + '.jm-heat{display:grid;gap:2px;font-size:.6rem}.jm-heat div{height:16px;border-radius:2px;display:grid;place-items:center;color:rgba(0,0,0,.75);font-weight:700}.jm-heat .lbl{color:var(--slate-400);background:none;justify-content:flex-start;font-weight:600}'
  ...
  // ---------- écrans ----------
    var GUIDE = {
    scenario:    ['1. Décrire la situation', 'Réglez les dons, les commandes et le stock de départ de votre région, puis cochez les événements à tester (une pandémie, une canicule, une campagne…). Le panneau « Avant de lancer » vous dit déjà si la semaine type est équilibrée. Lancez ensuite la simulation.'],
    resultats:   ['2. Lire les résultats', 'La simulation a rejoué la période choisie plusieurs fois, avec des aléas différents. Chaque chiffre est donc accompagné de sa marge d’incertitude. Regardez d’abord le groupe sanguin le plus tendu, puis les poches manquantes et la péremption.'],
    comparaison: ['3. Comparer deux situations', 'Mettez côte à côte une situation de référence et une variante, par exemple avec et sans campagne. Une différence est retenue seulement quand les marges d’incertitude ne se recouvrent pas.'],
    poste:       ['4. Revivre une journée', 'Le poste de régulation rejoue une journée simulée heure par heure : stock par groupe, commandes en attente, poches qui périment, événements du jour. Il montre à quel moment la tension apparaît.'],
    pheno:       ['Rechercher un profil sanguin', 'Cherchez les poches compatibles avec un groupe et un profil détaillé (Rhésus, Kell, Duffy) par site de collecte, sur la carte de la région.']
  };
  function nav(){ var V=[['scenario','1 · Scénario'],['resultats','2 · Résultats'],['comparaison','3 · Comparaison'],['poste','4 · Poste de régulation'],['pheno','Phénotypes']]; var g = GUIDE[UI.vue]; return '<div class="jm-intro"><b>Le jumeau numérique</b> reproduit sur ordinateur la chaîne du sang de la région : les dons qui arrivent, les poches qualifiées puis stockées, les commandes des hôpitaux, les poches qui périment. Il sert à répondre à une question du type « que se passerait-il si… » avant de décider.</div><div class="jm-nav">'+V.map(function(v){ return '<button data-v="'+v[0]+'" class="'+(UI.vue===v[0]?'on':'')+'">'+v[1]+'</button>'; }).join('')+'</div>'+(g?'<div class="jm-guide"><b>'+g[0]+'</b>'+g[1]+'</div>':''); }

  function vueScenario(){
    var sc = UI.sc, init = stockInitial(sc, UI.reel);
    var champs = [['nom','Nom de la situation','text','Donnez un nom parlant, par exemple « Été avec campagne ».'],['donsJour','Dons par jour','number','Moyenne d’un jour de semaine. Les samedis et dimanches suivent le profil hebdomadaire de la région.'],['ajournement','Ajournements (%)','number','Part des personnes qui se présentent et repartent sans donner (hémoglobine basse, voyage, traitement).'],['demandeJour','Commandes par jour (poches)','number','Poches demandées chaque jour par l’ensemble des hôpitaux de la région, toutes urgences confondues.'],['dureeVie','Durée de conservation (jours)','number','42 jours pour un concentré de globules rouges.'],['qualifH','Délai de qualification (heures)','number','Temps habituel entre le prélèvement et la mise en stock, après les analyses.'],['cibleJours','Stock de départ (jours de commandes)','number','Le stock initial couvre ce nombre de jours de commandes. Il est ajusté par le baromètre officiel du jour.'],['horizon','Durée simulée (jours)','number','Période sur laquelle les résultats sont mesurés.'],['chauffe','Mise en régime (jours)','number','Jours simulés avant le début de la mesure, pour que le stock atteigne un état stable. Ils ne comptent pas dans les résultats.'],['graine','Numéro de tirage','number','Fixe le hasard de la simulation. Le même numéro redonne exactement les mêmes résultats, ce qui permet de comparer deux situations à conditions égales.'],['reps','Nombre de simulations','select','La période est rejouée ce nombre de fois avec des aléas différents. Dix suffit pour explorer, trente pour conclure.'],['substitution','Substitution ABO','select','Autorise à servir une commande avec un groupe compatible quand le groupe demandé manque.']];
    var form = champs.map(function(c){ var v = sc[c[0]], inp; if(c[2]==='select' && c[0]==='reps') inp = '<select data-k="reps">'+[10,30,100].map(function(n){ return '<option'+(sc.reps===n?' selected':'')+'>'+n+'</option>'; }).join('')+'</select>'; else if(c[2]==='select') inp = '<select data-k="substitution">'+[['jamais','jamais'],['urgences','seulement pour les commandes vitales et urgentes'],['toujours','pour toutes les commandes']].map(function(o){ return '<option value="'+o[0]+'"'+(sc.substitution===o[0]?' selected':'')+'>'+o[1]+'</option>'; }).join('')+'</select>'; else inp = '<input data-k="'+c[0]+'" type="'+c[2]+'" value="'+h(v)+'">'; return '<div><label>'+c[1]+'</label>'+inp+'<small>'+c[3]+'</small></div>'; }).join('');
    ...
    var episodes = Object.keys(EPISODES).map(function(k){ var def = EPISODES[k], actif = sc.episodes.filter(function(e){return e.type===k;})[0]; return '<div class="jm-ep"><label><input type="checkbox" data-ep="'+k+'"'+(actif?' checked':'')+'><div><b>'+def.nom+'</b><span>'+def.desc+'</span></div></label>'+(actif?'<div class="jm-form">'+def.champs.map(function(c){ var v=actif.p[c[0]], t=c[3]||'nombre', inp; if(t==='jourSemaine') inp='<select data-ep="'+k+'" data-p="'+c[0]+'">'+JOURS.map(function(j,i){ return '<option value="'+i+'"'+(v===i?' selected':'')+'>'+j+'</option>'; }).join('')+'</select>'; else if(t==='groupe') inp='<select data-ep="'+k+'" data-p="'+c[0]+'">'+GROUPES.map(function(g,i){ return '<option value="'+i+'"'+(v===i?' selected':'')+'>'+g+'</option>'; }).join('')+'</select>'; else inp='<input type="number" data-ep="'+k+'" data-p="'+c[0]+'" value="'+v+'">'; return '<div><label>'+c[1]+(t==='jour'?' (jours après aujourd’hui)':'')+'</label>'+inp+(t==='jour'?'<small>le '+dateAxe(v)+'</small>':'')+'</div>'; }).join('')+'</div>':'')+'</div>'; }).join('');
    var runs = UI.runs.length ? '<div class="jm-wrap"><table class="jm-table"><thead><tr><th>N°</th><th>Situation</th><th>Rejouée</th><th>Tirage</th><th></th></tr></thead><tbody>'+UI.runs.map(function(r){ return '<tr><td>'+r.id+'<br><span class="jm-muted">'+r.quand.toLocaleTimeString('fr-CH',{hour:'2-digit',minute:'2-digit'})+'</span></td><td>'+h(r.nom)+'<br><span class="jm-muted">'+r.sc.horizon+' j · '+r.region+'</span></td><td>'+r.n+' fois</td><td>'+r.sc.graine+'</td><td><button class="jm-btn s" data-lire="'+r.id+'">Lire</button> <button class="jm-btn s" data-repartir="'+r.id+'">Créer une variante</button></td></tr>'; }).join('')+'</tbody></table></div>' : '<p class="jm-muted">Aucune simulation pour l’instant. La première lancée devient la référence des comparaisons.</p>';
    return '<div class="jm-grid"><div>'
      + '<div class="card"><div class="card-title">Situation de référence</div><p class="jm-muted" style="margin:-.4rem 0 .7rem">Les valeurs ci-dessous décrivent le fonctionnement habituel de la région. Elles sont préremplies avec des ordres de grandeur ; ajustez-les si vous connaissez les vôtres.</p><div class="jm-form">'+form+'</div>'
      + '<div class="jm-h">Avant de lancer</div><p class="jm-muted" style="margin-bottom:.5rem">Trois repères calculés directement à partir des valeurs saisies, sans simulation. Ils donnent un premier avis sur la situation.</p><div class="jm-grid3"><div><div class="jm-lbl">Dons (vert) et commandes (orange), par jour de semaine</div><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px">'+barres+'</div><p class="jm-note">'+totDons+' dons contre '+totDem+' poches commandées par semaine. '+(deficit.length? 'Les commandes dépassent les dons le '+deficit.join(', ')+'.' : 'Les dons couvrent les commandes chaque jour.')+'</p></div>'
      + '<div><div class="jm-lbl">Jours de couverture au départ, par groupe'+(UI.reel?' (baromètre officiel du '+h(UI.reelDate)+')':' (stock cible)')+'</div>'+couv+'</div>'
      + '<div><div class="jm-lbl">Jours à risque de pénurie</div><b style="font-size:1.6rem;color:'+(rouges?'#ef4444':'#22c55e')+'">'+rouges+'</b><p class="jm-note">Jours, sur la durée simulée, où au moins un groupe descendrait sous '+sc.seuilCritique+' jours de couverture si tout se passait « en moyenne ».</p></div></div>'
      + '<details class="jm-limits"><summary>Précautions de lecture</summary><ul><li>Les rythmes de dons et de commandes sont des ordres de grandeur de démonstration. Le pilote avec les données réelles du service de transfusion les remplacera.</li><li>Le stock de départ reprend les niveaux du baromètre officiel quand ils sont disponibles. La suite de la simulation est indépendante du stock réel.</li><li>La simulation ne déclenche aucune action réelle. Convoquer des donneur·se·s reste une décision de l’équipe.</li></ul></details></div>'
      + '</div><div>'
      + '<div class="sidebar-card"><h4 style="font-weight:700;margin-bottom:.5rem">Événements à simuler</h4><p class="jm-muted" style="margin-bottom:.5rem">Cochez un ou plusieurs événements. Chacun modifie temporairement les dons, les commandes ou le laboratoire à partir de la date indiquée, comptée en jours après aujourd’hui. Les événements se cumulent.</p>'+episodes+'</div>'
      + '<div class="sidebar-card" style="margin-top:1rem"><button class="jm-btn p" id="jm-go" style="width:100%">Lancer la simulation</button><div class="jm-bar"><i id="jm-prog"></i></div><p class="jm-muted" id="jm-msg">'+sc.horizon+' jours mesurés, rejoués '+sc.reps+' fois. Comptez quelques secondes.</p></div>'
      + '<div class="sidebar-card" style="margin-top:1rem"><h4 style="font-weight:700;margin-bottom:.5rem">Simulations de cette session</h4>'+runs+'</div>'
      + '</div></div>';
  }

  function vueResultats(){
    var r = UI.courant; if(!r) return '<div class="card"><p class="jm-muted">Aucune simulation à lire. Décrivez une situation dans l’écran Scénario, puis lancez la simulation.</p></div>';
    ...
    var kpis = '<div class="jm-kpis">'
      + '<div class="jm-kpi"><span>Groupe sanguin le plus tendu</span><div class="jm-arc" style="--p:'+pct+';--c:'+couleurCouv(cp,sc)+'"><i>'+pire+'</i></div><small style="text-align:center">'+f1(cp)+' jours de couverture ± '+f1(k.couv[pire].ic)+'</small></div>'
      + '<div class="jm-kpi"><span>Poches manquantes</span><b>'+pm(k.missingTot)+'</b><small>commandes servies en retard ou pas du tout, sur '+sc.horizon+' jours · environ '+Math.round(k.missingTot.moy*267).toLocaleString('fr-CH')+' CHF d’achats hors canton</small></div>'
      + '<div class="jm-kpi"><span>Péremption</span><b>'+pm(k.perim,' %')+'</b><small>part des poches mises en stock qui périment sans être utilisées</small></div>'
      + '<div class="jm-kpi"><span>Commandes urgentes servies à temps</span><b>'+f1((k.service[1].moy+k.service[2].moy)/2)+' %</b><small>'+f1(k.service[1].moy)+' % des commandes vitales · '+f1(k.service[2].moy)+' % des urgentes</small></div>'
      + '<div class="jm-kpi"><span>Dons mis en stock</span><b>'+Math.round(k.dons.moy).toLocaleString('fr-CH')+'</b><small>'+Math.round(k.demandes.moy).toLocaleString('fr-CH')+' commandes · '+Math.round(k.subst.moy)+' poches servies par substitution</small></div></div>';
    var table = '<div class="jm-wrap"><table class="jm-table"><thead><tr><th>Groupe</th><th>Couverture moyenne</th><th>Niveau le plus bas</th><th>Jours en tension (sous '+sc.seuilTension+' j)</th><th>Poches manquantes</th><th>Durée d’une pénurie</th></tr></thead><tbody>'
      + GROUPES.map(function(g){ var c=k.couv[g]; return '<tr><td><span class="jm-dot" style="background:'+COUL[g]+'"></span>'+g+'</td><td style="color:'+couleurCouv(c.moy,sc)+'">'+pm(c,' j')+'</td><td>'+f1(k.cMin[g].p10)+' j</td><td>'+pm(k.sous[g])+'</td><td>'+pm(k.missing[g])+'</td><td>'+(k.reappro[g].n? 'en général '+f1(k.reappro[g].med)+' j, au pire '+f1(k.reappro[g].p90)+' j' : 'aucune pénurie')+'</td></tr>'; }).join('')+'</tbody></table></div>';
    var semaines = Math.ceil(sc.horizon/7), heat = '<div class="jm-heat" style="grid-template-columns:44px repeat('+semaines+',1fr)"><div class="lbl"></div>'+Array.apply(null,Array(semaines)).map(function(_,w){ return '<div class="lbl" style="justify-content:center" title="Semaine '+(w+1)+', du '+dateAxe(w*7)+' au '+dateAxe(w*7+6)+'">'+(semaines<=26?'Sem. ':'S')+(w+1)+'</div>'; }).join('')
    ...
    return '<div class="card"><div class="jm-cart">Simulation <b>'+r.id+'</b> · '+h(r.nom)+' · '+r.region+' · '+sc.horizon+' jours mesurés, rejoués '+r.n+' fois · tirage n° '+sc.graine+' · '+r.quand.toLocaleString('fr-CH')+'<br>Chaque chiffre est la moyenne des '+r.n+' simulations, suivie de sa marge d’incertitude (±). La valeur réelle se situe dans cette marge dix-neuf fois sur vingt. Cohérence interne vérifiée.</div>'
      + kpis
      + '<div class="jm-h">Stock par groupe, jour après jour</div><p class="jm-muted" style="margin-bottom:.4rem">La ligne est la valeur la plus probable ; la zone colorée est la fourchette dans laquelle le stock se trouve huit fois sur dix. Les zones rouge et orange marquent les seuils de couverture. L’axe du temps part d’aujourd’hui, premier jour mesuré, et avance jour par jour.</p><div style="display:flex;gap:.5rem;align-items:center;margin-bottom:.4rem"><select class="jm-in" id="jm-g" style="width:auto">'+GROUPES.map(function(g){ return '<option'+(UI.groupe===g?' selected':'')+'>'+g+'</option>'; }).join('')+'</select><span class="jm-muted">Seuils : '+sc.seuilTension+' j tendu, '+sc.seuilCritique+' j critique.</span></div><canvas class="jm-canvas" id="jm-c1"></canvas>'
      + '<div class="jm-h">Couverture par semaine et par groupe</div><p class="jm-muted" style="margin-bottom:.4rem">Vert : plus de '+sc.seuilTension+' jours de couverture. Orange : tension. Rouge : moins de '+sc.seuilCritique+' jours, risque de pénurie. Une colonne par semaine simulée, en partant d’aujourd’hui.</p>'+heat
      + '<div class="jm-h">Détail par groupe sanguin</div><p class="jm-muted" style="margin-bottom:.4rem">Une pénurie commence quand la couverture passe sous '+sc.seuilCritique+' jours et dure jusqu’au retour au-dessus de ce seuil.</p>'+table
      + '<div class="jm-actions" style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.8rem"><button class="jm-btn s" id="jm-ref">Utiliser comme référence</button><button class="jm-btn s" id="jm-var">Utiliser comme variante</button><button class="jm-btn s" id="jm-poste">Revivre une journée</button><button class="jm-btn s" id="jm-exp">Exporter (JSON)</button></div>'
      + '<details class="jm-limits"><summary>Précautions de lecture</summary><ul><li>Le résultat décrit ce que la filière produirait dans les conditions décrites. Il ne prédit pas ce qui arrivera.</li><li>Les groupes rares reposent sur peu de poches, leurs marges d’incertitude sont donc larges.</li><li>Deux valeurs dont les marges se recouvrent ne se distinguent pas, même si les moyennes diffèrent.</li><li>Le coût des poches manquantes suppose un achat hors canton à 267 CHF par concentré.</li></ul></details></div>';
  }
  ...
  function dessinerSerie(){
    ...
    ctx.fillStyle = '#8b8b9e'; ctx.font = '10px DM Sans, sans-serif'; ctx.textAlign='right'; [0,.5,1].forEach(function(f){ ctx.fillText(Math.round(max*f), pl-4, y(max*f)+3); }); ctx.textAlign='center'; for(var d=0; d<n; d+=Math.max(1,Math.round(n/8))){ ctx.fillText(dateAxe(d), x(d), Hh-6); }
  ...
  function etatsComparaison(a, b, sens){ // sens : +1 plus grand est mieux, -1 plus petit est mieux, 0 non déclaré
    if(Math.abs(a.moy-b.moy)<1e-9 && Math.abs(a.ic-b.ic)<1e-9) return ['identiques','Identiques','nd'];
  ...
  function vueComparaison(){
    var sel = function(id, cur){ return '<select class="jm-in" id="'+id+'"><option value="">Choisir une simulation</option>'+UI.runs.map(function(r){ return '<option value="'+r.id+'"'+(cur&&cur.id===r.id?' selected':'')+'>'+r.id+' · '+h(r.nom)+'</option>'; }).join('')+'</select>'; };
    var head = '<div class="card"><div class="card-title">Comparer deux situations</div><div class="jm-form" style="grid-template-columns:1fr 1fr"><div><label>Référence</label>'+sel('jm-cref',UI.ref)+'</div><div><label>Variante</label>'+sel('jm-cvar',UI.variante)+'</div></div>';
    if(!UI.ref||!UI.variante) return head+'<p class="jm-muted" style="margin-top:.6rem">Choisissez une simulation de référence et une variante. Une différence est colorée en vert ou en rouge seulement quand les deux marges d’incertitude ne se recouvrent pas ; sinon elle est « dans la marge », trop faible pour conclure.</p></div>';
    var A=UI.ref, B=UI.variante, cadrage = (A.sc.horizon!==B.sc.horizon||A.sc.graine!==B.sc.graine||A.n!==B.n) ? '<p class="jm-muted" style="margin:.5rem 0;color:var(--amber-400)">Les deux simulations ont été lancées dans des conditions différentes (durée, numéro de tirage ou nombre de simulations). Une partie des différences ci-dessous peut venir du hasard.</p>' : '<p class="jm-muted" style="margin:.5rem 0">Les deux simulations ont été lancées dans les mêmes conditions. Les différences ci-dessous viennent uniquement de ce qui a été changé entre les deux.</p>';
    var LIBS = { donsJour:'dons par jour', ajournement:'ajournements', demandeJour:'commandes par jour', dureeVie:'durée de conservation', qualifH:'délai de qualification', cibleJours:'stock de départ', horizon:'durée simulée', chauffe:'mise en régime', graine:'numéro de tirage', reps:'nombre de simulations', substitution:'substitution ABO', seuilCritique:'seuil critique', seuilTension:'seuil de tension', profilDons:'profil hebdomadaire des dons', profilDemande:'profil hebdomadaire des commandes' };
    var diff = []; Object.keys(B.sc).forEach(function(k){ if(k!=='episodes'&&k!=='nom'&&JSON.stringify(A.sc[k])!==JSON.stringify(B.sc[k])) diff.push(LIBS[k]||k); });
    var evA = A.sc.episodes.map(function(e){return EPISODES[e.type].nom;}), evB = B.sc.episodes.map(function(e){return EPISODES[e.type].nom;});
    var ajoutes = evB.filter(function(x){return evA.indexOf(x)<0;}), retires = evA.filter(function(x){return evB.indexOf(x)<0;});
    var parts = []; if(diff.length) parts.push('les réglages suivants : '+diff.join(', ')); if(ajoutes.length) parts.push('l’ajout de l’événement « '+ajoutes.join(' », « ')+' »'); if(retires.length) parts.push('le retrait de l’événement « '+retires.join(' », « ')+' »');
    var changes = parts.length ? 'Ce qui change dans la variante par rapport à la référence : '+parts.join(' ; ')+'.' : 'La variante a exactement les mêmes réglages et les mêmes événements que la référence.';
    var lignes = [['Couverture du groupe le plus tendu ('+A.pire+' / '+B.pire+')', A.kpi.couv[A.pire], B.kpi.couv[B.pire], 1, ' j'],['Poches manquantes', A.kpi.missingTot, B.kpi.missingTot, -1, ''],['Péremption', A.kpi.perim, B.kpi.perim, -1, ' %'],['Commandes vitales servies à temps', A.kpi.service[1], B.kpi.service[1], 1, ' %'],['Commandes urgentes servies à temps', A.kpi.service[2], B.kpi.service[2], 1, ' %'],['Poches servies par substitution', A.kpi.subst, B.kpi.subst, -1, ''],['Dons mis en stock', A.kpi.dons, B.kpi.dons, 0, ''],['Commandes', A.kpi.demandes, B.kpi.demandes, 0, '']];
    ...
    return head+cadrage+'<p class="jm-muted">'+h(changes)+'</p>'
      + '<div class="jm-wrap" style="margin-top:.6rem"><table class="jm-table"><thead><tr><th>Mesure</th><th>Référence ('+A.id+')</th><th style="text-align:center">Lecture de l’écart</th><th>Variante ('+B.id+')</th></tr></thead><tbody>'+rows+'</tbody></table></div>'
      + '<div class="jm-h">Stock total, jour après jour</div><p class="jm-muted" style="margin-bottom:.4rem">L’axe du temps part d’aujourd’hui et avance jour par jour sur la durée simulée.</p><canvas class="jm-canvas" id="jm-c2"></canvas><div class="jm-legend"><span><i class="jm-dot" style="background:#94a3b8"></i>Référence</span><span><i class="jm-dot" style="background:#E8D5A3"></i>Variante</span><span>La zone entre les deux courbes est verte quand la variante a plus de stock, rouge quand elle en a moins.</span></div></div>';
  }
  ...
  function vuePoste(){
    var p = UI.poste, r = p.run || UI.courant; if(!r||!r.trace) return '<div class="card"><p class="jm-muted">Aucune simulation à revivre. Lancez-en une depuis l’écran Scénario, puis revenez ici.</p></div>';
    ...
    return '<div class="jm-grid"><div><div class="card"><div style="display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:.5rem"><div><div class="jm-clock">'+(p.heure<10?'0':'')+p.heure+'h00</div><div class="jm-muted">Jour '+(jour+1)+' sur '+sc.horizon+' · '+dateJour(jour)+' · simulation '+r.id+'. Journée simulée, sans lien avec le stock réel du jour.</div></div>'
      ...
      + '<div class="jm-kpis"><div class="jm-kpi"><span>Groupe sanguin le plus tendu</span><div class="jm-arc" style="--p:'+Math.min(100,Math.round(cp/sc.cibleJours*100))+';--c:'+couleurCouv(cp,sc)+'"><i>'+pire+'</i></div><small style="text-align:center">'+f1(cp)+' j de couverture</small></div><div class="jm-kpi"><span>Poches disponibles</span><b>'+total+'</b><small>tous groupes</small></div><div class="jm-kpi"><span>Commandes en attente</span><b>'+attente+'</b><small>non servies à cette heure</small></div><div class="jm-kpi"><span>Périment sous 24 h</span><b>'+perimDemain+'</b><small>'+GROUPES.filter(function(g){return hier.perimentDemain[g];}).map(function(g){return g+' '+hier.perimentDemain[g];}).join(' · ')+'</small></div><div class="jm-kpi"><span>Commandes du jour</span><b>'+Math.round(sc.demandeJour*sc.profilDemande[(jour+sc.chauffe)%7])+'</b><small>poches attendues, '+JOURS[(jour+sc.chauffe)%7]+'</small></div></div>'
      + '<div class="jm-h">Stock total au fil de la journée</div><p class="jm-muted" style="margin-bottom:.4rem">Trait plein jusqu’à l’heure affichée, trait discontinu pour la suite de la journée.</p><canvas class="jm-canvas" id="jm-c3"></canvas>'
      + '</div></div><div><div class="sidebar-card"><h4 style="font-weight:700;margin-bottom:.4rem">Stock par groupe à cette heure</h4><p class="jm-muted" style="margin-bottom:.4rem">Chaque point vaut un tiers de jour de couverture ; le nombre à droite est le nombre de poches.</p>'+jetons+'</div>'
      ...
      + '<div class="sidebar-card" style="margin-top:1rem"><h4 style="font-weight:700;margin-bottom:.4rem">Et si… ?</h4><p class="jm-muted">Et si la collecte de '+JOURS[(jour+sc.chauffe+2)%7]+' était annulée ? Et si une campagne pour le groupe '+pire+' partait demain ? Et si un hôpital doublait ses commandes ? Chaque question se traduit par un événement dans l’écran Scénario, puis se compare à la situation de référence.</p><button class="jm-btn s" id="jm-versSc" style="margin-top:.4rem">Créer une variante de cette situation</button></div></div></div>';
  }
  ...
  function rendre(){
    ...
    if(UI.vue==='scenario'){
      ...
      host.querySelectorAll('input[type=checkbox][data-ep]').forEach(function(c){ c.onchange=function(){ var k=c.dataset.ep; UI.sc.episodes=UI.sc.episodes.filter(function(e){return e.type!==k;}); if(c.checked){ var p={}; EPISODES[k].champs.forEach(function(ch){ p[ch[0]]=ch[2]; }); UI.sc.episodes.push({type:k,p:p}); if(UI.sc.nom==='Situation de référence') UI.sc.nom=EPISODES[k].nom; } rendre(); }; });
      host.querySelectorAll('input[type=number][data-ep][data-p], select[data-ep][data-p]').forEach(function(i){ i.onchange=function(){ var ep=UI.sc.episodes.filter(function(e){return e.type===i.dataset.ep;})[0]; if(ep){ ep.p[i.dataset.p]=Number(i.value); rendre(); } }; });
  ...
  function lancer(){
    ...
    executer(sc, UI.reel, function(f){ if(prog) prog.style.width=Math.round(f*100)+'%'; if(msg) msg.textContent='Passage '+Math.round(f*sc.reps)+' sur '+sc.reps+'…'; }, function(run){
      ...
      if(window.showToast) showToast('success', 'Simulation '+run.id+' terminée en '+((Date.now()-t1)/1000).toFixed(1)+' s. Groupe le plus tendu : '+run.pire+'.');
    });
  }
